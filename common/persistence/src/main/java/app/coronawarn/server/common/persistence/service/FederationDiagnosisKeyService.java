...
import app.coronawarn.server.common.persistence.domain.FederationDiagnosisKey;
import app.coronawarn.server.common.persistence.repository.FederationDiagnosisKeyRepository;
...
@Component
public class FederationDiagnosisKeyService {

  private static final Logger logger = LoggerFactory.getLogger(FederationDiagnosisKeyService.class);
  private final FederationDiagnosisKeyRepository keyRepository;

  public FederationDiagnosisKeyService(FederationDiagnosisKeyRepository keyRepository) {
    this.keyRepository = keyRepository;
  }

  /**
   * Persists the specified collection of {@link FederationDiagnosisKey} instances.
   * If the key data of a particular diagnosis key
  ...
  @Timed
  @Transactional
  public void saveDiagnosisKeys(Collection<FederationDiagnosisKey> diagnosisKeys) {
    for (FederationDiagnosisKey diagnosisKey : diagnosisKeys) {
      keyRepository.saveDoNothingOnConflict(
          ...
          diagnosisKey.getVisitedCountries(), diagnosisKey.getOrigin(), diagnosisKey.getVerificationType());
    }
  }


  private static boolean isDiagnosisKeyValid(FederationDiagnosisKey diagnosisKey) {
    Collection<ConstraintViolation<FederationDiagnosisKey>> violations = diagnosisKey.validate();
