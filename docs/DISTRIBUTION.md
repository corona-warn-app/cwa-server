...
The `origin_country` (ISO-3166 country code) parameter defines which country folder is being created. The origin country folder contains only the distributable keys for the origin country. Furthermore, based on the parameter`eu-package-name` an additional folder will be created which provides all keys from the supported countries in a single package. 
