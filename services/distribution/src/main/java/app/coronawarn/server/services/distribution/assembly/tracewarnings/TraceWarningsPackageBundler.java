...
public class TraceWarningsPackageBundler implements DistributionPackagesBundler<TraceTimeIntervalWarning> {
  ...
  /**
   * A map containing diagnosis keys, mapped by the LocalDateTime on which they may be distributed.
   */
  ...
  public File<WritableOnDisk> createTemporaryExportFile(List<TraceTimeIntervalWarning> data, String country,
    ...
    final double currentHourTime = this.distributionTime.toEpochSecond(ZoneOffset.UTC) / 3600.0;
    int intervalNumber = (int) Math.floor(currentHourTime);
    return TraceWarningPackageExportFile
        .fromTraceTimeIntervalWarnings(data, country, intervalNumber, distributionServiceConfig);
  }
