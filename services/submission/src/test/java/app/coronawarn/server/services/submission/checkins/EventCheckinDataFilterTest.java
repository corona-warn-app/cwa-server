...
class EventCheckinDataFilterTest {
  ...
  public void setup() {
    ...
    underTest = new EventCheckinDataFilter(mockConfig, createMockTranmissionRiskLevelMappingProvider());
  }
  ...
  void should_filter_out_checkins_for_old_events(int daysInThePast) {
    ...
    long eventCheckoutInThePast = LocalDateTime.ofInstant(thisTimeInstant, UTC).minusDays(daysInThePast)
        .toEpochSecond(UTC);

    long acceptableEventCheckoutDate = LocalDateTime.ofInstant(thisTimeInstant, UTC).minusDays(1).toEpochSecond(UTC);

    List<CheckIn> checkins = List.of(
        CheckIn.newBuilder().setStartIntervalNumber(TEN_MINUTE_INTERVAL_DERIVATION.apply(eventCheckoutInThePast - 10))
  ...
  void should_filter_out_checkins_which_map_to_zero_trl() {
    Instant thisTimeInstant = Instant.now();
    long eventCheckoutInThePast = LocalDateTime.ofInstant(thisTimeInstant, UTC).minusDays(10).toEpochSecond(UTC);
    ...
    assertEquals(filteredCheckin.getEndIntervalNumber(), TEN_MINUTE_INTERVAL_DERIVATION.apply(eventCheckoutInThePast));
    assertEquals(filteredCheckin.getTransmissionRiskLevel(), 3);
  }
  ...
  void should_filter_out_checkins_for_future_events() {
    Instant thisTimeInstant = Instant.now();
    long eventCheckinInTheFuture = LocalDateTime.ofInstant(thisTimeInstant, UTC).plusMinutes(11).toEpochSecond(UTC);

    long eventCheckinInTheNearPast = LocalDateTime.ofInstant(thisTimeInstant, UTC).minusMinutes(10).toEpochSecond(UTC);

    List<CheckIn> checkins = List.of(
        CheckIn.newBuilder().setStartIntervalNumber(TEN_MINUTE_INTERVAL_DERIVATION.apply(eventCheckinInTheFuture))
            .setEndIntervalNumber(TEN_MINUTE_INTERVAL_DERIVATION.apply(eventCheckinInTheFuture + 5))
            .setTransmissionRiskLevel(1).build(),
        CheckIn.newBuilder().setStartIntervalNumber(TEN_MINUTE_INTERVAL_DERIVATION.apply(eventCheckinInTheNearPast))
