...
public class ValidSubmissionOnBehalfTest {
  ...
  void shouldReturnTrueIfSubmissionOnBehalfPayloadIsValid() {
    ...
    when(eventCheckinDataValidator.verifySubmissionOnBehalf(validSubmissionPayload, constraintValidatorContext))
        .thenReturn(true);
    when(protectedReportsValidator.verifySubmissionOnBehalf(validSubmissionPayload, constraintValidatorContext))
        .thenReturn(true);
